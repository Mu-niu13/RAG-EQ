import json

# 情绪编号到情绪名称的映射
emotion_map = {
    0: "no emotion",
    1: "anger",
    2: "disgust",
    3: "fear",
    4: "happiness",
    5: "sadness",
    6: "surprise"
}

# 行为编号到行为名称的映射
act_map = {
    1: "inform",
    2: "question",
    3: "directive",
    4: "commissive"
}

# 话题编号到英文名称的映射
topic_map = {
    1: "Ordinary Life",
    2: "School Life",
    3: "Culture & Education",
    4: "Attitude & Emotion",
    5: "Relationship",
    6: "Tourism",
    7: "Health",
    8: "Work",
    9: "Politics",
    10: "Finance"
}

# 打开文件
with open('dialogues_text.txt', 'r', encoding='utf-8') as f_text, \
     open('dialogues_emotion.txt', 'r', encoding='utf-8') as f_emotion, \
     open('dialogues_act.txt', 'r', encoding='utf-8') as f_act, \
     open('dialogues_topic.txt', 'r', encoding='utf-8') as f_topic, \
     open('output.jsonl', 'w', encoding='utf-8') as f_out:

    # 遍历四个文件的每一行
    for text_line, emo_line, act_line, topic_line in zip(f_text, f_emotion, f_act, f_topic):
        # 清洗数据
        utterances = [u.strip() for u in text_line.strip().split('__eou__') if u.strip()]
        emotions = [int(e) for e in emo_line.strip().split()]
        acts = [int(a) for a in act_line.strip().split()]
        topic_id = int(topic_line.strip())
        topic = topic_map.get(topic_id, "Unknown")

        if len(utterances) != len(emotions) or len(utterances) != len(acts):
            continue  # 跳过标签不一致的行

        n = len(utterances)
        i = 0
        while i < n:
            if acts[i] == 2:  # 如果当前是一个 question
                query_emotion_id = emotions[i]
                query_emotion = emotion_map[query_emotion_id]

                # 向前收集 query（同情绪，act不能是directive或commissive）
                query_indices = [i]
                j = i - 1
                while j >= 0 and emotions[j] == query_emotion_id and acts[j] != 3 and acts[j] != 4:
                    query_indices.insert(0, j)
                    j -= 1

                # 向后收集 response（同情绪，不能以问号结尾，act不能是question）
                response_indices = []
                response_emotion_id = None
                k = i + 1
                while k < n and not utterances[k].endswith("?") and acts[k] != 2:
                    if response_emotion_id is None:
                        response_emotion_id = emotions[k]
                    if emotions[k] == response_emotion_id:
                        response_indices.append(k)
                        k += 1
                    else:
                        break

                if response_indices:
                    sample = {
                        "topic": topic,
                        "query": " ".join([utterances[idx] for idx in query_indices]),
                        "query_emotion": query_emotion,
                        "response": " ".join([utterances[idx] for idx in response_indices]),
                        "response_emotion": emotion_map[response_emotion_id]
                    }

                    # 存入JSON文件
                    f_out.write(json.dumps(sample, ensure_ascii=False) + "\n")

                i = k  # 跳过已经处理过的部分
            else:
                i += 1